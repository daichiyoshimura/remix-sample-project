...
const Alert = ({ type, message, onClose }: AlertProps) => {
  return (
    <div className={`bg-${type === 'success' ? 'green' : type === 'error' ? 'red' : 'yellow'}-100 border border-${type === 'success' ? 'green' : type === 'error' ? 'red' : 'yellow'}-400 text-${type === 'success' ? 'green' : type === 'error' ? 'red' : 'yellow'}-700 px-4 py-3 rounded relative`} role="alert">
      <strong className="font-bold">{type === 'success' ? 'Success!' : type === 'error' ? 'Error!' : 'Warning!'}</strong>
